...
Reader.loadDatatablesArchives = async function (datatablesPage) {
    ...
    const newonly = selectedCategories.includes("NEW_ONLY") ? 1 : 0;
    const untaggedonly = selectedCategories.includes("UNTAGGED_ONLY") ? 1 : 0;

    const body = {
        clauses: [{ filter: indexSearchQuery.trim(), categories, newonly, untaggedonly }],
        start: (datatablesPage - 1) * datatablesPageSize,
        sortby: (indexSort && indexSort !== '0') ? (localStorage[`customColumn${indexSort}`] || "title") : "title",
        order: indexOrder,
        groupby_tanks: true,
    };

    const searchUrl = new LRR.apiURL("/api/search/composite");
    console.debug("Using Composite Search API URL:", searchUrl.toString());
...
Reader.returnToIndex = function () {
    ...
    for (const catId of selectedCategories) params.append('c', catId);
