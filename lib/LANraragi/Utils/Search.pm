...
use Exporter 'import';
our @EXPORT_OK = qw(reduce_clauses normalize_clauses compute_search_filter);
...
sub normalize_clauses ($clause_descriptors) {
    ...
    for my $desc (@$clause_descriptors) {
        ...
        my @canon_cats   = sort map { $_->{id} . ":" . $_->{mode} } @{ $desc->{categories} };

        push @normed, {
            tokens         => \@canon_tokens,
            raw_tokens     => \@tokens,
            categories     => \@canon_cats,
            raw_categories => $desc->{categories},
...
sub reduce_clauses ($normed) {
    ...
    for my $i ( 0 .. $#$normed ) {
        ...
        for my $j ( 0 .. $#$normed ) {
            ...
            my $subsumes = 1;
            for my $flag (qw(newonly untaggedonly)) {
