...
//! # Playbook
//!
//! 1. **Per-layer error ownership.** `db::*` returns `sqlx::Error` directly.
//!    `support::*` and `service::*` define their own enums for non-trivial
//!    work. Controllers return `Result<T, ApiError>`.
//! 2. **Variants name domain outcomes**, not which library failed.
//! 3. **Boilerplate is hand-rolled** (`Display`, `Error`). No `thiserror`.
//! 4. **Propagate explicitly** within services/support via `?` + `map_err`.
//!    At the controller boundary, `tag_err` invokes the
//!    `Into<PendingApiError>` bridge and attaches the operation id in one
//!    step. Bare `?` against a service result in a controller fails to
//!    compile — only `tag_err` produces the final `ApiError`.
//! 5. **Exhaustive matching** happens once per domain enum, inside its
//!    `From<DomainError> for PendingApiError` impl. New variants force
//!    re-compilation of the bridge, not every controller using it.
//! 6. **4xx error strings** are composed by services in plain language.
//!    **5xx error strings** may include the underlying `Display`.
//! 7. **No `anyhow` or `Box<dyn Error>`** at module boundaries. Permitted
//!    only in `main` and inline `#[cfg(test)]`.
//! 8. **Logging.** 5xx errors log at `warn!`/`error!` with correlation ids.
//!    4xx errors do not log. `catch_unwind` boundaries always log at
//!    `error!`.
//! 9. **`ApiError` is the only `IntoResponse` on the error path.** No
//!    handler hand-constructs `(StatusCode, Json<Value>)` tuples.
//! 10. **Domain bridges live next to the domain.** `From<UploadError> for
//!     PendingApiError` is in `service/archive.rs`, not here.
//! 11. **`tag_err` at every wire-emitting site.** Compile-time enforced via
//!     the [`PendingApiError`] → [`ApiError`] split.
